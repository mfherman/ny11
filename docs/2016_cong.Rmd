...
tmap_mode("view")
map <- tm_shape(cong_2016_to_map, name = "2016 Congressional") +
  tm_fill(
    col = "margin",
    palette = "RdBu",
    style = "cont",
...
tmap_leaflet(map) %>%
  addSearchFeatures(
    targetGroups = "2016 Congressional",
...
to_table <- cong_2016_margin %>%
  ...
  select(elect_dist, margin, lean, total_votes,
         reichard_prop, donovan_prop, everything()) %>%
  arrange(margin)
...
datatable(
  ...
  colnames = c("Election District", "Margin", "Lean", "Total Votes",
               "Reichard %", "Donovan %", "Reichard", "Donovan"),

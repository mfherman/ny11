...
opts_chunk$set(echo = FALSE, cache = TRUE, cache.path = here("cache/2008_pres/"))
```

```{r process data, include = FALSE}
# read in cd 11 ed shape file
cd11_ed_sf <- read_sf(here("output/geo/cd11_ed_2008.geojson"))

# read in pres results 2008
pres_2008 <- read_csv(
  here("output/elect_results/2008_pres.csv"),
...
pres_2008_margin <- pres_2008 %>%
  mutate(margin = obama_prop - mccain_prop) %>%
  select(elect_dist, total_votes, margin, obama_tot,
         mccain_tot, obama_prop, mccain_prop)

pres_2008_to_map <- left_join(cd11_ed_sf, pres_2008_margin) %>%
  mutate(elect_dist = paste("ED", elect_dist))

pres_2008_close_rep <- pres_2008_to_map %>%
  filter(margin >= -0.1 & margin < 0)

pres_2008_close_dem <- pres_2008_to_map %>%
...
# make a map of obama/mccain margin 2008, highlight ed +-10pt
tmap_mode("view")
map <- 
  tm_shape(pres_2008_to_map, name = "2008 Presidential") +
  tm_fill(
    col = "margin",
    palette = "RdBu",
    style = "cont",
    ...
    popup.format = list(
      ...
      mccain_prop = list(fun = make_pct)
      )
  ) +
  tm_borders(col = "darkgray") +
  tm_shape(pres_2008_close_dem, name = "Lean Dem") +
  tm_borders(col = "orange", lwd = 2) +
  tm_shape(pres_2008_close_rep, name = "Lean Rep") +
...
tmap_leaflet(map) %>%
  addSearchFeatures(
    targetGroups = "2008 Presidential",
...
to_table <- pres_2008_margin %>%
  ...
  select(elect_dist, margin, lean, total_votes,
         obama_prop, mccain_prop, everything()) %>%
  arrange(margin)
